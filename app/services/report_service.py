...
def calculate_categories(parking_areas, date):
    ...
    for area in parking_areas:
        ...
        grouped_vehicle_count = time_interval.groupby("garageCode")[
            "vehicleCount"
        ].mean()
        grouped_total_spaces = time_interval.groupby("garageCode")[
            "totalSpaces"
        ].first()
...
def get_all_reports():
    with open("app/data/reports.csv") as file:
        ...
        for row in reader:
            reports.append(
                {"id": row[0], "name": row[1], "parking_areas": row[2], "date": row[3]}
            )
    return reports
